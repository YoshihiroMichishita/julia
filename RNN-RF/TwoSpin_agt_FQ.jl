...
function init_nQ(en::TS_env, n::Int=512, γ0::Float64=0.95, ϵ0::Float64=1.0e-3)
    ...
    HF_TL = zeros(Float64, 2en.N_size+1, en.HS_size^2)
    K_TL = zeros(Float64, 2en.N_size+1, en.HS_size^2)

    return in_size, out_size, n_dense, ϵ, γ, HF_TL, K_TL
end

function calc_HF_n(en::TS_env, ag::agtQ)
...
function micro_motion2(Kp_t::Vector{Float32}, K_t::Vector{Float64}, en::TS_env, t::Int)
    #dt = (2pi/en.t_size/en.Ω)
    Kt = VtoM(K_t, en)
    U = exp(1.0im*Kt)
    K_t_new = K_t + en.dt * Kp_t
...
function loss_calc_new(model0, en::TS_env, ag::agtQ, HF_given::Vector{Float64})
    l::Float64 = 0.0
    kp_sum = zeros(Float64, en.HS_size^2)
    for t in 1:en.t_size
        if(t==1)
