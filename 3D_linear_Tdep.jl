...
function main(arg::Array{String,1})
    ...
    for j in 1:length(mu0)
        ...
        p = Parm(parse(Float64,arg[1]), pi/2, parse(Float64,arg[2]), parse(Float64,arg[3]), parse(Float64,arg[4]), parse(Float64,arg[5]), mu0[j], parse(Int,arg[6]), parse(Float64,arg[7]), parse(Int,arg[8]))
