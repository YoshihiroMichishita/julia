...
function main(arg::Array{String,1})
    ...
    #gr()
    ENV["GKSwstype"]="nul"
    p1=plot(mu0, DrudeX_mu, label="Drude",xlabel="μ",ylabel="σ",title="nonlinear conductivity", width=2.0, marker=:circle)
    p1=plot!(mu0, BCX_mu+dQMX_mu, label="sQM", width=2.0, marker=:circle)
    p1=plot!(mu0, app_QM_mu, label="app_QM", width=2.0, marker=:circle)
    p1=plot!(mu0, Drude0_mu+app_QM_mu, label="tot0", width=2.0, marker=:circle)
