...
function main(arg::Array{String,1})
    ...
    for j in 1:length(T0)
        #Parm: m, lamda, hx, hy, hz, mu, eta, T, K_MAX, K_SIZE, W_in, W_MAX, W_SIZE
        p = Parm(parse(Float64,arg[1]), parse(Float64,arg[2]), 0.0, 0.3, 0.0, T0[j], parse(Float64,arg[3]), parse(Float64,arg[4]), 1.0, parse(Int,arg[5]), parse(Float64,arg[6]), parse(Float64,arg[7]), parse(Int,arg[8]))
        if j == 1
            println(p)
        end
    ...
    p2=plot(T0, Drude_XYY_mu, label="Drude",xlabel="μ",ylabel="σ",title="nonlinear conductivity", width=2.0, marker=:circle)
