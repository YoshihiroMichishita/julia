...
@everywhere function PV_calcu_ver(p::Parm, k::Float64, eta::Float64)
    ...
    for q in collect(-pi:dk:pi-dk)
        ...
        for w in collect(-p.W_MAX:dw:10p.T)
            Gkw = Green(Gk(w,Hk,p,eta)...)
            for wp in collect(-5.0p.T:dw:10p.T)
                Gqw = Green(Gk(w+wp,Hq,p,eta)...)
                DRq, DAq = Gp(wp,q,p)
                Jxxx += dk * dw * imag(tr(Hk.Vx * Gkw.GR * Gqw.GR * Hq.Vx * (Gqw.GRp + Gqw.GRm) * Hq.Vx *Gqw.GR * DRq * Gkw.GRmA)) * f(w,p.T) *b(wp,p.T)
                Jxxx += dk * dw * imag(tr(Hk.Vx * Gkw.GR * Gqw.GA * Hq.Vx * (Gqw.GAp + Gqw.GAm) * Hq.Vx *Gqw.GA * DAq * Gkw.GRmA)) * f(w,p.T) *b(wp,p.T)
                Jxxx += dk * dw * imag(tr(Hk.Vx * Gkw.GR * Gqw.GR * Hq.Vx * (Gqw.GRp + Gqw.GRm) * Hq.Vx *Gqw.GR * DRq * Gkw.GA)) * f(w+wp,p.T) *b(wp,p.T)
