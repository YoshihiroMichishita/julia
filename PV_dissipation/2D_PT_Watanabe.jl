...
function main(arg::Array{String,1})
    ...
    Win0 = collect(0.1:0.1:parse(Float64,arg[11]))
    ...
    for j in 1:size(Win0)[1]
        ...
        Green_W[j,:], Velocity_W[j,:], Length_W[j,:] = @distributed (+) for i in 1:size(kk)[1]
            Hamk = Hamiltonian(HandV_fd(kk[i],p)...)
            Gr_PV = Green_PV_BI(p, Hamk)
            Vel_PV = Velocity_PV_BI(p, Hamk)
            Len_PV = Length_PV_BI(p, Hamk)


            dk2*Gr_PV, dk2*Vel_PV, dk2*Len_PV
        end
        print("#")
        if(j%10==0)
            print("||")
    ...
    CSV.write("./Wdep_η"*p.eta*"_"*arg[14]*arg[15]*arg[16]*"_Green.csv", save_data1)

    save_data2 = DataFrame(Velocity_W, :auto)
    CSV.write("./Wdep_η"*p.eta*"_"*arg[14]*arg[15]*arg[16]*"_Velocity.csv", save_data2)

    save_data3 = DataFrame(Length_W, :auto)
    CSV.write("./Wdep_η"*p.eta*"_"*arg[14]*arg[15]*arg[16]*"_Length.csv", save_data3)
    ...
    savefig(p1,"./PV_Linear_Wdep_η"*p.eta*"_"*arg[14]*arg[15]*arg[16]*".png")
    ...
    savefig(p2,"./PV_CP_Wdep_η"*p.eta*"_"*arg[14]*arg[15]*arg[16]*".png")
end
