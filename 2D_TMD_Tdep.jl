...
function main(arg::Array{String,1})
    ...
    mu0 = [0.002, 0.004, 0.006, 0.008, 0.01, 0.012, 0.014, 0.016, 0.018, 0.02, 0.025, 0.03, 0.035, 0.04, 0.045, 0.05, 0.06, 0.07, 0.08, 0.09, 0.1, 0.12, 0.14, 0.16]
    ...
    for j in 1:length(mu0)
        ...
        p = Parm(0.5, 0.08, 0.06, 0.7, parse(Float64,arg[1]), parse(Float64,arg[2]), mu0[j], 0.0, 0.0, 0.0, parse(Int,arg[3]), 3.0, parse(Int,arg[4]))
    ...
    CSV.write("./Tdep_YXX.csv", save_data2)

    ENV["GKSwstype"]="nul"
    Plots.scalefontsizes(1.4)
    p1 = plot(mu0, -Drude_XXX_mu, label="eBC",xlabel="T",ylabel="σ",title="T-dependence", width=4.0, marker=:circle, markersize = 4.8)
