...
function main(arg::Array{String,1})
    ...
    ZXYG_mu = zeros(Float64,length(T0))
    ZXYSC_mu = zeros(Float64,length(T0))
    #A::String = []
    for j in 1:length(T0)
        ...
        for kz in collect(-p.K_MAX:2*p.K_MAX/p.K_SIZE:p.K_MAX)[1:end-1]
            ZXYG::Float64 = 0.0
            ZXYSC::Float64 = 0.0
            ZXYG, ZXYSC = @distributed (+) for i in 1:length(k2)
                k = (k2[i][1], k2[i][2], kz)
                Hamk = Hamiltonian_3D(HandV_weyl(k,p)...)
                ZXY0 = Green_PV_NLH(p,Hamk)
                ZXYSC0 = SC_PV_NLH(p, Hamk)
                [ZXY0/(p.K_SIZE^3), ZXYSC0/(p.K_SIZE^3)]
            end
            ZXYG_mu[j] += ZXYG
            ZXYSC_mu[j] += ZXYSC
        end
    ...
    save_data = DataFrame(T=T0,ZXY_YX_Green=ZXYG_mu,ZXY_YX_SC=ZXYSC_mu)
    #「./」で現在の(tutorial.ipynbがある)ディレクトリにファイルを作成の意味、指定すれば別のディレクトリにファイルを作ることも出来る。
    CSV.write("./T_dep_NLH.csv", save_data)

    #gr()
    ENV["GKSwstype"]="nul"
    p = plot(T0, ZXYG_mu, label="Green",xlabel="T",ylabel="τσ",title="nonlinear conductivity", width=2.0, marker=:circle)
    p = plot!(T0, ZXYSC_mu, label="SC", width=2.0, marker=:circle)
