...
function set_H(k::Vector{Float64},p::Parm)
    eps0::Float64 = t_i*cos(k[2]) + p.mu
    t_x::Float64 = t_i*cos(k[1])
    t_y::Float64 = t_b*sin(k[1])
    t_z::Float64 = Δ 
    ee = [eps0, t_x, t_y, t_z]
    H = ee' * sigma
    return H
...
function Disp_HSL(p::Parm)
    E = zeros(CopmlexF64, 4*p.K_SIZE, 2)

    for K0 in 1:p.K_SIZE
        KK = pi*K0/p.K_SIZE
        kk = [KK, 0.0]
        E[K0,:], v = eigen(set_H(kk, p))
    end
    for K0 in 1:p.K_SIZE
        KK = pi*K0/p.K_SIZE
        kk = [pi-KK, KK]
        E[K0+p.K_SIZE,:], v = eigen(set_H(kk, p))
    end
    for K0 in 1:p.K_SIZE
        KK = pi*K0/p.K_SIZE
        kk = [0.0, pi-KK]
        E[K0+2p.K_SIZE,:], v = eigen(set_H(kk, p))
    end
    for K0 in 1:p.K_SIZE
        KK = pi*K0/p.K_SIZE
        kk = [KK, KK]
        E[K0+3p.K_SIZE,:], v = eigen(set_H(kk, p))
    end
