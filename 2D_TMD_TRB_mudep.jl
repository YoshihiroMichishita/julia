...
@everywhere function Green_YYY_M(p::Parm, H::Hamiltonian)
    G0::Float64 = 0.0
    #mi = minimum([p.W_MAX,12p.T])
    #dw::Float64 = mi/p.W_SIZE/pi
    for w in 0:p.W_SIZE
        G = Green(G_M(w,p,H)...)
        G0 += 2.0imag(1.0im*tr(H.Vy*G.ddGR*(2.0*H.Vy*G.GR*H.Vy + H.Vyy)*G.GRmA))
        G0 += 2.0imag(1.0im*tr(H.Vy*G.dGR*(2.0*H.Vy*G.dGR*H.Vy)*G.GRmA))
        G0 += 2.0imag(1.0im*tr(H.Vy*G.dGR*(2.0*H.Vy*G.GR*H.Vy + H.Vyy)*(G.dGR-G.dGA)))
    end
    return p.T*G0
end
...
@everywhere function Green_YXX_M(p::Parm, H::Hamiltonian)
    G0::Float64 = 0.0
    #mi = minimum([p.W_MAX,12p.T])
    #dw::Float64 = mi/p.W_SIZE/pi
    for w in 0:p.W_SIZE
        G = Green(G_M(w,p,H)...)
        G0 += 2.0imag(1.0im*tr(H.Vy*G.ddGR*(2.0*H.Vx*G.GR*H.Vx + H.Vxx)*G.GRmA))
        G0 += 2.0imag(1.0im*tr(H.Vy*G.dGR*(2.0*H.Vx*G.dGR*H.Vx)*G.GRmA))
        G0 += 2.0imag(1.0im*tr(H.Vy*G.dGR*(2.0*H.Vx*G.GR*H.Vx + H.Vxx)*(G.dGR-G.dGA)))
    end
    return p.T*G0
end
...
function main(arg::Array{String,1})
    ...
    CSV.write("./mudep_YYYY_T002_M.csv", save_data2)

    ENV["GKSwstype"]="nul"
    Plots.scalefontsizes(1.4)
    p1 = plot(mu0, Green_XXX_M, label="Drude",xlabel="μ",ylabel="σ",title="μ-dependence", width=4.0, marker=:circle, markersize = 4.8)
    ...
    #p1 = plot!(mu0, Green_XXX_mu+(Drude_XXX_mu+sQMD_XXX_mu+BCD_XXX_mu+dQMD_XXX_mu+Inter_XXX_mu+dInter_XXX_mu), label="TF", width=4.0, marker=:circle, color=:green, markersize = 4.8)

    #p1 = plot!(mu0, Green_XXX_mu, label="Green", width=4.0, marker=:circle, color=:red, markersize = 4.8)
    #p1 = plot!(mu0, Green_XXX_sea, label="Green_sea", width=4.0, marker=:circle, markersize = 4.8)
    savefig(p1,"./mudep_YYYY_M.png")
end
