...
function main(arg::Array{String,1})
    ...
    save_data2 = DataFrame(W_in=mu0, PV_simple=PV_XXX_mu, PV_ver=PV_XXX_ver_mu)
    CSV.write("./PV_Wdep_XXX_T002.csv", save_data2)
    ...
    p2 = plot(mu0, PV_XXX_mu, label="w/o vertex",xlabel="Ω",ylabel="PV",title="Ω-dependence", width=4.0, marker=:circle, markersize = 4.8)
    p2 = plot!(mu0, PV_XXX_ver_mu, label="vertex", width=4.0, marker=:circle, markersize = 4.8)
    savefig(p2,"./Wdep_PV_XXX.png")
end
