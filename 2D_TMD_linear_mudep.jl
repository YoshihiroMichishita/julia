...
function main(arg::Array{String,1})
    ...
    Green_YY_mu = zeros(Float64,length(mu0))

    Drude_YY_mu = zeros(Float64,length(mu0))
    Drude0_YY_mu = zeros(Float64,length(mu0))
    dQMre_YY_mu = zeros(Float64,length(mu0))
    dQMim_YY_mu = zeros(Float64,length(mu0))
    dQM_app_mu = zeros(Float64,length(mu0))
    ...
    for j in 1:length(mu0)
        ...
        #else
            p = Parm(0.5, 0.08, 0.06, 0.7, mu0[j], parse(Float64,arg[1]), parse(Float64,arg[2]), 0.05, 0.0, 0.0, parse(Int,arg[3]), 3.0, parse(Int,arg[4]))
        #end
        ...
        Drude_YY_mu[j], Drude0_YY_mu[j], dQMre_YY_mu[j], dQMim_YY_mu[j], dQM_app_mu[j], Green_YY_mu[j] = @distributed (+) for i in 1:length(kk)
            Hamk = Hamiltonian(HandV(kk[i],p)...)
            Green = Green_DC_2D(p, Hamk)
            HV_BI(Hamk)
            Drude, Drude0, dQMre, dQMim, dQM_app = Green_YY2_BI(p,Hamk)
            [dk2*Drude, dk2*Drude0, dk2*dQMre, dk2*dQMim, dk2*dQM_app, dk2*Green]
        end
        print("#")
    end
    println("finish the calculation!")
    # headerの名前を(Q,E1,E2)にして、CSVファイル形式を作成
    save_data2 = DataFrame(mu=mu0, Drude=Drude_YY_mu, dQM_re=dQMre_YY_mu, dQM_im=dQMim_YY_mu, dQM_app=dQM_app_mu, Green=Green_YY_mu)
    CSV.write("./etadep_YY.csv", save_data2)

    ENV["GKSwstype"]="nul"
    Plots.scalefontsizes(1.4)
    p1 = plot(mu0, Drude_YY_mu, label="Drude",xlabel="η",ylabel="σ",title="η-dependence", width=4.0, marker=:circle, markersize = 4.8, gridwidth=2.0)
    #plot!(mu0, sQMD_XXX_mu, label="sQMD", width=2.0, marker=:circle)
    p1 = plot!(mu0, dQMre_YY_mu, label="dQM_re", width=4.0, marker=:circle, markersize = 4.8)
    p1 = plot!(mu0, dQMim_YY_mu, label="dQM_im", width=4.0, marker=:circle, markersize = 4.8)
    p1 = plot!(mu0, Drude0_YY_mu+dQM_app_mu, label="app", width=4.0, marker=:circle, markersize = 4.8)
    ...
    p1 = plot!(mu0, Green_YY_mu-(Drude_YY_mu+dQMre_YY_mu+dQMDim_YY_mu), label="Sm", width=4.0, marker=:circle, color=:green, markersize = 4.8)

    p1 = plot!(mu0, Green_YY_mu, label="Green", width=4.0, marker=:circle, color=:red, markersize = 4.8)
    savefig(p1,"./eta_dep_YY.png")
end
