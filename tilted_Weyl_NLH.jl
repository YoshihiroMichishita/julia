...
function main(arg::Array{String,1})
    ...
    for j in 1:length(mu0)
        #Parm: m, lamda, hx, hy, hz, mu, eta, T, K_MAX, K_SIZE, W_in, W_MAX, W_SIZE
        p = Parm(parse(Float64,arg[1]), parse(Float64,arg[2]), 0.0, 0.0, 0.0, mu0[j], parse(Float64,arg[3]), parse(Float64,arg[4]), 1.0, parse(Int,arg[5]), parse(Float64,arg[6]), parse(Float64,arg[7]), parse(Int,arg[8]))
    ...
    save_data = DataFrame(mu=mu0,ZXY_YX=ZXY_mu,XYZ_ZY=XYZ_mu)
