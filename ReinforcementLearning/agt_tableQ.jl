...
function check_and_add_obs(qagt::TableQAgt, q::Q_table, obs::String)
    if(!haskey(q.Q,obs))
        q.Q[obs] = qagt.init_val_Q * ones(Float64, qagt.n_act)
        q.len_Q += 1
...
function mainQ(arg::Array{String,1})
    ...
    println("structの初期化完了")
    obs = reset(en,st)

    for t in 1:n_step
        act = decide_action(agt,q,obs)
    ...
    for obs0 in obss
        q_vals = get_Q(q,obs0)
        if(!isnothing(q_vals))
            println(obs0*": "*string(q.Q[obs0][1])*", "*string(q.Q[obs0][2]))
        else
            println(obs0*": ")
        end
    end
