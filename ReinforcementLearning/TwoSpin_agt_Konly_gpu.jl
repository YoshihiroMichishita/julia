...
function loss_calc_hyb(model0, en::TS_env, ag::agtQ, HF_given::Vector{Float32})
    ...
    l += 10.0*diff_norm(kp_sum,en)/en.t_size
    return l 
end
...
function main(arg::Array{String,1})
    ...
    #println(ll_it[1])
    #dt = gpu(2pi/en.Ω/en.t_size*Matrix{Float32}(I,2,2))
    #HF_old = CUDA.zeros(en.HS_size^2)
    #HF_new = CUDA.zeros(en.HS_size^2)
    #Kt = CUDA.zeros(en.HS_size^2)
    for it in 1:it_MAX
        HF_it = zeros(Float32, en.HS_size^2)
        #HF_old = CUDA.zeros(en.HS_size^2)
        #HF_new = CUDA.zeros(en.HS_size^2)
        #Kt = CUDA.zeros(en.HS_size^2)
        #Kt_new = CUDA.zeros(Float32, en.HS_size^2) 
        if(it==1)
        ...
        ag.K_TL[en.t_size,:] = zeros(Float32, en.HS_size^2)
        ll_it[it], Kp_av = loss_calc_hyb!(model_g,en, ag, HF_it)
        ...
        #=
        if(it%10 == 0)
            ee = zeros(Float32, en.t_size, en.HS_size)
            for t_step in 1:en.t_size
