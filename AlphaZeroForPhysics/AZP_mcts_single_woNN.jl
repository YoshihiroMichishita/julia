...
function evaluate!(env::Env, agt::Agent,node::Node)
    
    A = legal_action(env, agt.history, agt.branch_left)
    policy = softmax([0.0f0 for a in A])
...
function run_MCTS(env::Env, agt::Agent)
    root = init_node(Float32(0.0))
    value = evaluate!(env, agt, root)
    ...
    for it in 1:env.num_simulation
        ...
        value = evaluate!(env, scratch, node)
...
function run_MCTS(env::Env, agt::Agent, ratio::Float32, noise_r::Float32)
    root = init_node(Float32(0.0))
    value = evaluate!(env, agt, root)
...
function run_MCTS!(env::Env, agt::Agent, ratio::Float32, noise_r::Float32, scores::Dict{Vector{Int}, Float32}, max_hist::Vector{Float32})
    root = init_node(Float32(0.0))
    value = evaluate!(env, agt, root)
    add_exploration_noise!(env, root, noise_r)
    for it in 1:env.num_simulation
...
function run_MCTS!(env::Env, agt::Agent, ratio::Float32, noise_r::Float32, storage::Storage, max_hist::Vector{Float32})
    root = init_node(Float32(0.0))
    value = evaluate!(env, agt, root)
    add_exploration_noise!(env, root, noise_r)
    for it in 1:env.num_simulation
...
function play_physics!(env::Env)
    agt = init_agt()
    while(!is_finish(env, agt))
        action, root = run_MCTS(env, agt)
...
function play_physics!(env::Env, ratio::Float32, noise_r::Float32)
    agt = init_agt()
    while(!is_finish(env, agt))
        action, root = run_MCTS(env, agt, ratio, noise_r)
        apply!(env, agt, action)
        store_search_statistics!(env, root, agt)
    end
    return agt
end
function play_physics!(env::Env, ratio::Float32, noise_r::Float32, scores::Dict{Vector{Int}, Float32}, max_hist::Vector{Float32})
    agt = init_agt()
    while(!is_finish(env, agt))
        action, root = run_MCTS!(env, agt, ratio, noise_r, scores, max_hist)
        apply!(env, agt, action)
        store_search_statistics!(env, root, agt)
    end
    return agt
...
function play_physics!(env::Env, ratio::Float32, noise_r::Float32, storage::Storage, max_hist::Vector{Float32})
    agt = init_agt()
    while(!is_finish(env, agt))
        action, root = run_MCTS!(env, agt, ratio, noise_r, storage, max_hist)
        apply!(env, agt, action)
        store_search_statistics!(env, root, agt)
    end
    return agt
