...
function loss(image::CuArray{Int, 2}, target::Matrix{Float32}, env::Env, model::Chain)
    y1 = cpu(model(image))
    return sum([(((y1[end,i]-target[end,i])/5)^2 - target[1:end-1,i]' * log.(softmax(y1[1:end-1,i]).+1f-8)) for i in 1:env.batch_size])/env.batch_size
    # + env.C * sum(sqnorm, Flux.params(model))
end


tanh10(x) = Float32(10)*tanh(x)
tanh2(x) = Float32(4)*tanh(x)
...
function main(args::Vector{String})
    ...
    for head in 1:env.batch_num
        ...
        @save "/home/yoshihiro/Documents/Codes/julia/AlphaZeroForPhysics/AZP_head$(st+head+20).bson" model0
    end
    #model2 = cpu(model)
